...
export class TaskService extends BaseService {
   ...
   */
  async start(params: StartTaskJobParams): Promise<StartTaskJobResponse> {
    try {
      return await this.request<StartTaskJobResponse>("/task", {
        method: "POST",
   ...
   */
  async getStatus(id: string): Promise<TaskJobStatusResponse> {
    try {
      return await this.request<TaskJobStatusResponse>(`/task/${id}/status`);
    } catch (error) {
      if (error instanceof HyperbrowserError) {
        throw error;
      }
   ...
   * @param id The ID of the task job to get
   */
  async get(id: string): Promise<TaskJobResponse> {
    try {
      return await this.request<TaskJobResponse>(`/task/${id}`);
    } catch (error) {
      if (error instanceof HyperbrowserError) {
        throw error;
      }
   ...
   * @param params The parameters for the task job
   */
  async startAndWait(params: StartTaskJobParams): Promise<TaskJobResponse> {
